...
import src.pickling as pickling
import src.reading_images as reading_images
import src.saving_images as saving_images
...
def get_mask_of_largest_connected_component(img_mask):
    ...
    """
    mask, mask_pixels_dict = get_masks_and_sizes_of_connected_components(
        img_mask)
...
def get_bottommost_pixels(img, largest_mask, y_edge_bottom):
    ...
    bottommost_nonzero_x = np.arange(
        img.shape[1])[largest_mask[bottommost_nonzero_y, :] > 0]
...
def convert_bottommost_pixels_wrt_cropped_image(mode, bottommost_nonzero_y, bottommost_nonzero_x,
    ...
    if mode == "left":
        # in this case, not in sorted order anymore.
        bottommost_nonzero_x = x_edge_right - bottommost_nonzero_x
...
def get_rightmost_pixels_wrt_cropped_image(mode, largest_mask_cropped, find_rightmost_from_ratio):
    ...
    """
    ignore_height = int(
        largest_mask_cropped.shape[0] * find_rightmost_from_ratio)
    rightmost_pixel_search_area = largest_mask_cropped[ignore_height:, :]
    rightmost_pixel_search_area_has_value = np.any(
        rightmost_pixel_search_area, axis=0)
    ...
    if mode == "left":
        rightmost_nonzero_x = largest_mask_cropped.shape[1] - \
            rightmost_nonzero_x
...
def crop_img_from_largest_connected(img, mode, erode_dialate=True, iterations=100,
    ...
    if erode_dialate:
        img_mask = scipy.ndimage.morphology.binary_erosion(
            img_mask, iterations=iterations)
    ...
    if erode_dialate:
        largest_mask = scipy.ndimage.morphology.binary_dilation(
            largest_mask, iterations=iterations)
    ...
    bottommost_nonzero_y, bottommost_nonzero_x = get_bottommost_pixels(
        img, largest_mask, y_edge_bottom)

    # include maximum 'buffer_size' more pixels on both sides just to make sure we don't miss anything
    y_edge_top, y_edge_bottom = include_buffer_y_axis(
        img, y_edge_top, y_edge_bottom, buffer_size)

    # If cropped image not starting from corresponding edge, they are wrong. Record the distance, will reject if not 0.
    distance_from_starting_side = get_distance_from_starting_side(
        img, mode, x_edge_left, x_edge_right)
    ...
    x_edge_left, x_edge_right = include_buffer_x_axis(
        img, mode, x_edge_left, x_edge_right, buffer_size)
...
def crop_mammogram(input_data_folder, exam_list_path, cropped_exam_list_path, output_data_folder,
    ...
    data_handling.add_metadata(
        exam_list, "window_location", window_location_dict)
    data_handling.add_metadata(
        exam_list, "rightmost_points", rightmost_points_dict)
    data_handling.add_metadata(
        exam_list, "bottommost_points", bottommost_points_dict)
    data_handling.add_metadata(
        exam_list, "distance_from_starting_side", distance_from_starting_side_dict)
...
def crop_mammogram_one_image(scan, input_file_path, output_file_path, num_iterations, buffer_size):
    ...
    image = reading_images.read_image_png_or_jpeg(input_file_path)
    try:
...
def crop_mammogram_one_image_short_path(scan, input_data_folder, output_data_folder,
    ...
    full_input_file_path = os.path.join(
        input_data_folder, scan['short_file_path']+'.png')
    full_output_file_path = os.path.join(
        output_data_folder, scan['short_file_path'] + '.png')
